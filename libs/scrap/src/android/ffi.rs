...
fn try_init_rustls_platform_verifier(env: &mut JNIEnv, context_jobject: *mut c_void) {
    use hbb_common::config::RUSTLS_PLATFORM_VERIFIER_INITIALIZED;
    use std::sync::atomic::Ordering;
    let initialized = RUSTLS_PLATFORM_VERIFIER_INITIALIZED.load(Ordering::Relaxed);
    if !initialized {
            ...
            hbb_common::rustls_platform_verifier::android::init_hosted(env, ctx_for_rustls)
        {
            ...
            RUSTLS_PLATFORM_VERIFIER_INITIALIZED.store(true, Ordering::Relaxed);
